...
public class event extends ScrollPane {
    ...
    public event() {
        ...
        kpiRow.getChildren().addAll(
                ...
                format.formatKPICard("Sắp tới", "3", "#3b82f6", "#1e293b"),
                format.formatKPICard("Đang diễn ra", "1", "#f59e0b", "#1e293b"),
                format.formatKPICard("Kết thúc", "18", "#10b981", "#1e293b")
        );
        ...
        ComboBox<String> cbStatus = format.formatSortBtn("Trạng thái", "Tất cả", "Sắp tới", "Đang diễn ra", "Kết thúc", "Hủy");
        ...
        eventRows.getChildren().addAll(
                createEventRow("Workshop GitHub", "28/06/2026", "08:00 - 11:30", "Hội trường A", "Truyền thông", "Sắp tới", "Sắp tới", "Chưa tham gia"),
                createEventRow("Họp toàn CLB", "26/06/2026", "19:00 - 21:00", "Google Meet", "Sự kiện", "Đang diễn ra", "Đang diễn ra", "Đã tham gia"),
                createEventRow("Team Building", "12/06/2026", "07:00 - 17:00", "Biển Mỹ Khê", "Nhân sự", "Kết thúc", "Kết thúc", "Đã tham gia")
        );
        ...
        Label logTitle = format.formatLabel("NHẬT KÝ THAM GIA", FontWeight.BLACK, 14, "#1e293b");
        ...
        if (joinLogRows.getChildren().isEmpty()) {
            addLogEntry("26/06/2026 19:15:30", "Đã tham gia: Họp toàn CLB");
        }
    ...
    private HBox createEventRow(String title, String date, String time, String loc, String leader, String status, String statusStr, String userJoinState) {
        ...
        Label lblTime = format.formatLabel(date + "\n" + time, FontWeight.MEDIUM, 12, "#475569"); lblTime.setPrefWidth(140); lblTime.setWrapText(true);
        ...
        String stBg = status.equals("Sắp tới") ? "rgba(59,130,246,0.15)" : status.equals("Đang diễn ra") ? "rgba(245,158,11,0.15)" : "rgba(16,185,129,0.15)";
        String stText = status.equals("Sắp tới") ? "#3b82f6" : status.equals("Đang diễn ra") ? "#f59e0b" : "#10b981";
        ...
        btnView.setOnAction(e -> {
            VBox modal = createEventDetailModal(title, date, time, loc, leader, status, 120, "Thông tin sự kiện.", userJoinState);
    ...
    private HBox createLogTableHeader() {
        ...
        Label l1 = format.formatLabel("THỜI GIAN", FontWeight.BLACK, 10, "#94a3b8"); l1.setPrefWidth(220);
        Label l2 = format.formatLabel("NỘI DUNG", FontWeight.BLACK, 10, "#94a3b8"); l2.setPrefWidth(650);
    ...
    private VBox createEventDetailModal(String title, String date, String time, String loc, String leader, String status, int participants, String desc, String initialJoinState) {
        ...
        cardDetails.getChildren().addAll(
                new VBox(4, format.formatLabel("Thời gian", FontWeight.BOLD, 11, "#94a3b8"), format.formatLabel(date + " | " + time, FontWeight.BOLD, 14, "#1e293b")),
                new VBox(4, format.formatLabel("Địa điểm", FontWeight.BOLD, 11, "#94a3b8"), format.formatLabel(loc, FontWeight.BOLD, 14, "#1e293b")),
                new VBox(4, format.formatLabel("Mô tả", FontWeight.BOLD, 11, "#94a3b8"), textDesc)
        );
        ...
        Label listTitle = format.formatLabel("SINH VIÊN THAM GIA", FontWeight.BLACK, 12, "#94a3b8");

        studentJoinRows.getChildren().clear();
        addStudentToMockRow("25GT044", "Nguyễn Văn Hùng");
        addStudentToMockRow("24SK012", "Trần Thị Mai Ly");
        ...
        userRealtimeRow.getChildren().addAll(
                format.formatLabel("25GT002", FontWeight.BOLD, 11, "#10b981"),
                format.formatLabel("Bạn", FontWeight.BOLD, 13, "#1e293b")
        );
        ...
        Button btnCancelJoin = getFormBtn("Hủy", "#ef4444", "white");
        ...
        btnJoin.setOnAction(e -> {
            ...
            invokeToast("Tham gia thành công!");
            String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"));
            addLogEntry(timestamp, "Đã tham gia: " + title);
        });

        btnCancelJoin.setOnAction(e -> {
            VBox confirmModal = format.formatSimpleModal(
                    "Xác nhận", "Hủy tham gia?", "Quay lại", "Xác nhận",
                    ev -> invokeCloseModal(),
                    ev -> {
                        invokeCloseModal();
                        btnCancelJoin.setVisible(false); btnCancelJoin.setManaged(false);
                        ...
                        invokeToast("Đã hủy.");
                        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"));
                        addLogEntry(timestamp, "Đã hủy: " + title);
                    }
            );
        ...
        actionRow.getChildren().addAll(btnClose, btnJoin, btnCancelJoin);
