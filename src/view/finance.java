...
public class finance extends ScrollPane {
    ...
    public finance() {
        ...
        kpiRow.getChildren().addAll(
                ...
                format.formatKPICard("Thu tháng này", "+3.500.000đ", "#3b82f6", "#1e293b"),
                format.formatKPICard("Chi tháng này", "-1.200.000đ", "#ef4444", "#1e293b"),
        ...
        logTitleBox.getChildren().addAll(
                ...
                format.formatLabel("Dữ liệu hệ thống, không thể thay đổi.", FontWeight.MEDIUM, 11, "#ef4444")
        );
        ...
        logRows.getChildren().addAll(
                createLogRow("28/06 14:35", "Quân", "Thu: Hội phí tháng 6 (+2.500.000đ)"),
                createLogRow("27/06 09:12", "Yến", "Chi: In ấn banner (-350.000đ)"),
                createLogRow("25/06 18:40", "Quân", "Hủy: Sai số tiền (-1.000.000đ)"),
                createLogRow("20/06 10:00", "Quân", "Thu: Tài trợ ABC (+5.000.000đ)"),
                createLogRow("15/06 08:30", "Yến", "Chi: Mua sắm (-1.000.000đ)")
        );
        ...
        tableContainer.getChildren().addAll(
                createTransRow("28/06/2026", "Thu", "Hội phí tháng 6", "+2.500.000đ", "Hợp lệ", "Không", "Nguyễn Minh Quân"),
                createTransRow("27/06/2026", "Chi", "In ấn banner", "-350.000đ", "Hợp lệ", "Workshop GitHub", "Hoàng Kim Yến"),
                createTransRow("20/06/2026", "Thu", "Tài trợ ABC", "+5.000.000đ", "Hợp lệ", "Workshop GitHub", "Nguyễn Minh Quân"),
                createTransRow("15/06/2026", "Chi", "Mua sắm vật tư", "-1.000.000đ", "Đã hủy", "Team Building", "Hoàng Kim Yến")
        );
    ...
    private HBox createTableHeader() {
        ...
        Label l6 = format.formatLabel("HÀNH ĐỘNG", FontWeight.BLACK, 10, "#94a3b8"); l6.setPrefWidth(60);
    ...
    private StackPane createDetailModal(String date, String type, String content, String amount, String status, String event, String executor, String note) {
        ...
        Label lblTitle = format.formatLabel("Thông tin", FontWeight.BLACK, 24, "#1e293b");
        Region spacer = new Region(); HBox.setHgrow(spacer, Priority.ALWAYS);
        ...
        if (status.equals("Hợp lệ")) {
            Button btnCancelTrans = getShadowBtn("Hủy giao dịch", "✖", "rgba(239,68,68,0.1)", "#ef4444", "rgba(0,0,0,0.1)");
    ...
    private VBox createTransactionFormModal(StackPane rootModalPane) {
        ...
        btnConfirm.setOnAction(e -> {
            if (fContent.getText().trim().isEmpty()) {
                frame.getInstance().triggerToast("Nhập nội dung giao dịch");
                return;
            }
            if (fAmount.getText().trim().isEmpty()) {
                frame.getInstance().triggerToast("Nhập số tiền");
                return;
            }
            frame.getInstance().closeOverlayModal();
            frame.getInstance().triggerToast("Đã thêm giao dịch");
        });
    ...
    private VBox createCancelModal(String transContent, StackPane rootModalPane, VBox previousView) {
        ...
        Label wDesc = format.formatLabel("Hệ thống sẽ hủy giao dịch và hoàn lại số dư.", FontWeight.MEDIUM, 12, "#dc2626");
        ...
        btnConfirm.setOnAction(e -> {
            if (fReason.getText().trim().isEmpty()) {
                frame.getInstance().triggerToast("Nhập lý do hủy");
                return;
            }
            frame.getInstance().closeOverlayModal();
